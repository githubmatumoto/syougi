...
static void
gen_kumi_pre_fu_aux (const int leng, int youso[][2],
		 BOARD_CLASS & list, int list_pos,
		     const int koma_nokori, const int masu_nokori)
{
  ...
  if ((ret = list.Set_Array (list_pos, (KOMA) youso[0][0])) > 0)
	...
	Board::best_koma_nokori = k;
      ...
      gen_kumi_pre_fu_aux (leng, youso, list, list_pos + 1, k, masu_nokori - ret);
  ...
  gen_kumi_pre_fu_aux (leng, youso, list, list_pos + 1, koma_nokori, masu_nokori);
}
		 ...
		 const int koma_nokori, const int masu_nokori)
{
  ...
  gen_kumi_pre_fu_aux (leng, youso, list, list_pos, koma_nokori, masu_nokori);
}
